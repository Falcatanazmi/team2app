...
object BezierSplineUtil {
    fun getCurveControlPoints(knots: Array<Point>): Array<Array<Point>> {
        ...
        val n = knots.size - 1
        val firstControlPoints = Array(n) { Point(0f, 0f) }
        val secondControlPoints = Array(n) { Point(0f, 0f) }

        // Special case: bezier curve should be a straight line
        if (n == 1) {
            // 3P1 = 2P0 + P3
            val x = (2 * knots[0].x + knots[1].x) / 3
            val y = (2 * knots[0].y + knots[1].y) / 3
            firstControlPoints[0] = Point(x, y)

            // P2 = 2P1 - P0
            val xx = 2 * firstControlPoints[0].x - knots[0].x
            val yy = 2 * firstControlPoints[0].y - knots[0].y
            secondControlPoints[0] = Point(xx, yy)
